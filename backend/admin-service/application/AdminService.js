...
class AdminService {
  ...
  async createDisease(payload, token) {
    const result = await this.serviceClient.createEncyclopediaArticle(payload, token);
    return result.data || result;
  }

  async updateDisease(diseaseId, payload, token) {
    const result = await this.serviceClient.updateEncyclopediaArticle(diseaseId, payload, token);
    return result.data || result;
  }
  ...
  async updateNotice(noticeId, payload, token) {
    const result = await this.serviceClient.updateNotice(noticeId, payload, token);
    return result.data || result;
  }
