...
class ServiceClient {
  ...
  async getUserInfo(token) {
    return this._request(`${this.userServiceUrl}/api/v1/users/me`, {
  ...
  async getDiagnosisLogs(token, page = 1, limit = 20) {
    return this._request(`${this.diagnosisServiceUrl}/api/v1/diagnoses/logs?page=${page}&limit=${limit}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  }
  ...
  async getArticles(page = 1, limit = 10) {
    return this._request(`${this.contentServiceUrl}/api/v1/content/encyclopedia?page=${page}&limit=${limit}`);
  }

  async getNotices(page = 1, limit = 10) {
    return this._request(`${this.contentServiceUrl}/api/v1/content/notices?page=${page}&limit=${limit}`);
  }

  // ── 피드백 서비스 통신 ────────────────────
  async getFeedbacks(token, page = 1, limit = 10) {
    return this._request(`${this.feedbackServiceUrl}/api/v1/feedbacks/me?page=${page}&limit=${limit}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  }
}
