...
class AdminRepositoryImpl extends AdminRepository {
  ...
  async updateUserStatus(userId, status) {
    return safe(async () => {
      ...
      const [rows] = await this.pool.execute(
        "SELECT user_id, name, email, status FROM users WHERE user_id = ?", [userId]
      );
  ...
  async findFeedbacks({ page = 1, limit = 10 }) {
    return safe(async () => {
      ...
      const [[cnt]] = await this.pool.query("SELECT COUNT(*) AS total FROM feedbacks");
      return {
        data: rows,
        pagination: { page, limit, total: cnt.total, totalPages: Math.ceil(cnt.total / limit) },
      };
  ...
  async findExamRecords({ page = 1, limit = 10, search }) {
    return safe(async () => {
      ...
      LEFT JOIN users u ON u.user_id = d.user_id ${where}`, params
      );
      return {
        data: rows,
        pagination: { page, limit, total: cnt.total, totalPages: Math.ceil(cnt.total / limit) },
    ...
    return safe(async () => {
      const [rows] = await this.pool.execute(
        `SELECT image_id, file_name, file_size, image_url, uploaded_at
           FROM images WHERE image_id = ? LIMIT 1`, [imageId]
      );
      return rows[0] || null;
    }, null);
  }
  ...
  async getUserTrend() {
    return safe(async () => {
      ...
      return rows.map(r => ({ month: r.month, new: Number(r.new), active: Number(r.new) }));
    }, []);
  }
