...
class AdminController {
  ...
  getUsers = async (req, res, next) => {
    try {
      ...
      const result = await this.adminService.getUsers({ page, limit, status, token: req.token });
  ...
  suspendUser = async (req, res, next) => {
    try {
      const data = await this.adminService.suspendUser(req.params.userId, req.token);
      res.status(200).json({ success: true, message: "사용자가 정지되었습니다.", data });
    } catch (e) { next(e); }
  };

  unsuspendUser = async (req, res, next) => {
    try {
      const data = await this.adminService.unsuspendUser(req.params.userId, req.token);
      res.status(200).json({ success: true, message: "사용자 정지가 해제되었습니다.", data });
    } catch (e) { next(e); }
  };
  ...
  getFeedbacks = async (req, res, next) => {
    try {
      const page = parseInt(req.query.page) || 1;
      const limit = parseInt(req.query.limit) || 10;
      const result = await this.adminService.getFeedbacks({ page, limit, token: req.token });
      res.status(200).json({ success: true, data: result.data || [], pagination: result.pagination || {} });
    } catch (e) { next(e); }
  };
  ...
  replyFeedback = async (req, res, next) => {
    try {
      ...
      const data = await this.adminService.replyFeedback(req.params.feedbackId, replyText, req.token);
      res.status(200).json({ success: true, message: "피드백 답변이 등록되었습니다.", data });
    } catch (e) { next(e); }
  };
  ...
  getExamRecords = async (req, res, next) => {
    try {
      ...
      const result = await this.adminService.getExamRecords({ page, limit, search, token: req.token });
      res.status(200).json({ success: true, data: result.data || [], pagination: result.pagination || {} });
    } catch (e) { next(e); }
  };
  ...
  getImageInfo = async (req, res, next) => {
    try {
      const data = await this.adminService.getImageInfo(req.params.imageId, req.token);
  ...
  getDiseaseDetail = async (req, res, next) => {
    try {
      const data = await this.adminService.getDiseaseDetail(req.params.diseaseId);
      res.status(200).json({ success: true, data });
    } catch (e) { next(e); }
  };
  ...
  createDisease = async (req, res, next) => {
    try {
      ...
      res.status(201).json({ success: true, message: "질환 정보가 등록되었습니다.", data });
    } catch (e) { next(e); }
  };

  updateDisease = async (req, res, next) => {
    try {
      const data = await this.adminService.updateDisease(req.params.diseaseId, req.body, req.token);
      res.status(200).json({ success: true, message: "질환 정보가 수정되었습니다.", data });
    } catch (e) { next(e); }
  };
  ...
  deleteDisease = async (req, res, next) => {
    try {
      ...
      res.status(200).json({ success: true, message: "질환 정보가 삭제되었습니다." });
    } catch (e) { next(e); }
  };
  ...
  createNotice = async (req, res, next) => {
    try {
      ...
      res.status(201).json({ success: true, message: "공지사항이 등록되었습니다.", data });
    } catch (e) { next(e); }
  };

  updateNotice = async (req, res, next) => {
    try {
      const data = await this.adminService.updateNotice(req.params.noticeId, req.body, req.token);
      res.status(200).json({ success: true, message: "공지사항이 수정되었습니다.", data });
    } catch (e) { next(e); }
  };
