...
class AuthService {
  ...
  async _createUserInUserService({ email, name }) {
    try {
      const response = await fetch(`${USER_SERVICE_URL}/api/v1/users/internal/create`, {
        method: "POST",
  ...
  async signup({ email, password, name }) {
    ...
    // 인증 정보 저장 (name도 캐시)
    const auth = new Auth({ user_id: userId, email, password: hashedPassword, name, role });
    await this.authRepository.save(auth);

    // 토큰 발급
    const { accessToken, refreshToken } = this._issueTokens({ userId, email, name, role });
