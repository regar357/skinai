...
class DiagnosisService {
  ...
  async deleteDiagnosis(id, userId) {
    ...
    await this.diagnosisRepository.saveLog(
      DiagnosisLog.create({ diagnosis_id: id, user_id: userId, action: "deleted", detail: "사용자 삭제" })
    );
    return true;
  }
  ...
  async deleteDiagnosesMany(ids, userId) {
    for (const id of ids) {
      ...
      if (!d) continue;
      if (d.user_id !== userId) {
        throw new DomainError(`진단 ID ${id}에 대한 접근 권한이 없습니다.`, 403);
      }
      await this.diagnosisRepository.deleteDiagnosis(id);
      await this.diagnosisRepository.saveLog(
        DiagnosisLog.create({ diagnosis_id: id, user_id: userId, action: "deleted", detail: "사용자 다건 삭제" })
      );
    }
    return true;
  }
