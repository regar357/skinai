...
class DiagnosisRepositoryImpl extends DiagnosisRepository {
  ...
  async countDiagnosesByUserId(userId) {
    const [rows] = await this.pool.execute(
      "SELECT COUNT(*) as total FROM diagnoses WHERE user_id = ?",
      [userId],
    );
    return rows[0].total;
  }
  ...
  async findImagesByDiagnosisId(diagnosisId) {
    const [rows] = await this.pool.execute(
      "SELECT * FROM images WHERE diagnosis_id = ?",
      [diagnosisId],
    );
  ...
  async findLogsByDiagnosisId(diagnosisId) {
    const [rows] = await this.pool.execute(
      "SELECT * FROM diagnosis_logs WHERE diagnosis_id = ? ORDER BY created_at DESC",
      [diagnosisId],
    );
  ...
  async countAllLogs() {
    const [rows] = await this.pool.execute(
      "SELECT COUNT(*) as total FROM diagnosis_logs",
    );
    return rows[0].total;
  }
