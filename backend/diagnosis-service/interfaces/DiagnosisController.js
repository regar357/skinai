...
class DiagnosisController {
  ...
  deleteOne = async (req, res, next) => {
    try {
      ...
      res.status(200).json({ success: true, message: "진단이 삭제되었습니다." });
    } catch (e) { next(e); }
  };
  ...
  deleteMany = async (req, res, next) => {
    try {
      ...
      if (ids.length === 0) {
        return res.status(400).json({ success: false, message: "삭제할 진단 ID 목록(ids)이 필요합니다." });
      }
      await this.service.deleteDiagnosesMany(ids, req.user.userId);
      res.status(200).json({ success: true, message: `${ids.length}건의 진단이 삭제되었습니다.` });
    } catch (e) { next(e); }
  };
