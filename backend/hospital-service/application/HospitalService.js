...
class HospitalService {
  ...
  async searchHospitals({ latitude, longitude, sort = "distance", page = 1, size = 3, radius = 5000, keyword }) {
    Hospital.validateCoordinates(latitude, longitude);

    // TODO: 네이버 지도 API 연동
    // - Redis 캐싱으로 자주 검색되는 지역 결과 캐싱 (비용 절감)
    // - 요청 횟수 제한 (Rate Limiting)
    const all = await this.hospitalRepository.findNearby(latitude, longitude, radius, keyword);
