class MonitoringClient {
  constructor() {
    this.diagnosisUrl = process.env.DIAGNOSIS_SERVICE_URL || "http://localhost:3004";
    this.aiUrl = process.env.AI_SERVICE_URL || "http://localhost:8001";
    this.internalToken = process.env.INTERNAL_SERVICE_TOKEN || "internal-dev-token";
  }
  ...
  async get(url) {
    try {
      ...
      console.warn("[monitoring-service] internal call failed:", url, e.message);
      return null;
    }
  }
  ...
  getDiseaseAccuracy() {
    return this.get(`${this.diagnosisUrl}/internal/monitoring/disease-accuracy`);
  }
