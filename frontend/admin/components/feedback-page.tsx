...
export function FeedbackPage() {
  ...
  const paginatedFeedback = filteredFeedback.slice(startIndex, startIndex + pageSize)
            ...
            <div className="space-y-6">
              {paginatedFeedback.map((feedback) => (
                <div key={feedback.id} className="border-b border-gray-100 pb-6 last:border-b-0 last:pb-0">
                  ...
                  <p className="mb-1 text-sm text-gray-700">{feedback.content}</p>
                  ...
                  {replyingTo === feedback.id && (
                    <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-4">
                      ...
                      <div className="mt-2 flex gap-2">
                        <Button onClick={handleSubmitReply} size="sm" className="bg-blue-500 hover:bg-blue-600">
                          답변 제출
                        </Button>
                        <Button onClick={handleCancelReply} size="sm" variant="outline">
                          취소
                        </Button>
                      </div>
                    </div>
          ...
          {filteredFeedback.length > pageSize && (
            <div className="flex items-center justify-between px-2 py-4">
              <div className="text-sm text-gray-700">
                총 {filteredFeedback.length}개 중 {startIndex + 1}-{Math.min(startIndex + pageSize, filteredFeedback.length)}개 표시
              </div>
